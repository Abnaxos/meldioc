...
In case of conflicts, we can use the usual Java language features, even
though that's a bit verbose and complicated (sorry about that):
...
@Configuration
abstract class MyContext implements MyFeature, AnotherFeature {

  @Mount
  abstract InternalMyFeatureImpl myFeature();

  @Mount
  abstract AnotherFeatureImpl anotherFeature();

  // if we don't override, we'll get an error from the annotation processor
  @Provision
  @Override
  public Foo foo() {
    // The problem here is that the generated code will override both foo()
    // methods to call this method. Therefore, we'd get a recursion if we'd
    // just write myFeature().foo(). So, we have to extend the feature to
    // provide access to the original implementation.
    return myFeature().superFoo();
    // we could also do wild things like this:
    // return new FooMultiplexer(
    //   myFeature().superFoo(),
    //   anotherFeature().superFoo());
  }

  @Feature
  static class InternalMyFeatureImpl extends MyFeatureImpl {
    Foo superFoo() {
      return super.foo();
    }
  }
...
- copy the acceptor's state when provisioning a component
